...
(function () {
	...
	Canvas.Img.prototype._setRandomProperties = function(oConfig) {
		...
		if (oConfig.top == null) {
			...
			this.top = this.height / 2 + 0.77 * 500;
		}
		
		if (oConfig.left == null) {
			//this.top = this.height / 2 + Math.random() * 500;
			this.left = this.width / 2 + 0.67 * 700;
		}
	};
	...
	Canvas.Img.prototype.getImageCoords = function() {
		return {
	 ...
	 */
	Canvas.Img.prototype.setImageCoords = function(setCoords = {}) {
		this.left = setCoords.left || parseInt(this.left);
		this.top = setCoords.top || parseInt(this.top);
		this.scalex = setCoords.scalex || this.scalex;
		this.scaley = setCoords.scaley || this.scaley;
		this.angle = setCoords.angle || this.angle;
		this.theta = setCoords.theta || this.theta;
